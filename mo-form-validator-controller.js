export class MoFormValidatorController {
    ...
    validateInput(input, form) {
        ...
        if (
            (input.getAttribute(`type`) === `date` || input.hasAttribute(`isdate`)) &&
            input.hasAttribute(`dateprevious`)
        ) {
    ...
    #ageCalculator(date, format) {
        ...
        let actualyear = dateAct.getYear();
        let actualMonth = dateAct.getMonth() + 1;
        let actualDay = dateAct.getDate();
     ...
     */
    #validateIsUrl(input) {
        const url = this.#getInputValue(input);
        if (!/^(http|https|ftp)\:\/\//.test(url) && url) {
            url = `http://` + url;
        }

        let regexp = /^(http|https|ftp)\:\/\/[a-z0-9\.-]+\.[a-z]{2,4}/gi;
    ...
    #validateNoSpaces(input) {
        let attr = input.getAttribute(`nospaces`);
    ...
    #validateIsMaxLength(input) {
        let maxlength = parseInt(input.getAttribute(`maxlength`) ?? 0);
        const value = this.#getInputValue(input);
    ...
    #validateDomains(input) {
        ...
        for (let i = 0; i < domains.length; i++) {
            let regexp = new RegExp(domains[i], `g`);
    ...
    #validateIsDate(input) {
        ...
        let arrDate = splittedDate[0].split(`-`);
        let arrHour = splittedDate[1] ? splittedDate[1].split(`:`) : null;
        ...
        let template = new Date(year, month - 1, day); //mes empieza de cero Enero = 0
        if (
            !template ||
            (template.getFullYear() !== year && template.getMonth() !== month - 1 && template.getDate() !== day)
        ) {
            return false;
        }
        ...
        if (arrHour) {
            let hour = parseInt(arrHour[0]);
            let minute = parseInt(arrHour[1]);
    ...
    #validateIsDateprevius(input) {
        ...
        let format = input.getAttribute(`isdate`) ?? `aaaa-mm-dd`;

        let dateSplitted = date.split(` `);
        let arrDate = dateSplitted[0].split(`-`);
    ...
    #validatePhonenumber(input) {
        ...
        let regexp = /^((\+\d{1,3})|(00\d{1,3}))?(\(\d{1,3}\))?([\d]){7,11}$/;
